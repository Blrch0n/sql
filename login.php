...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password)) {
        ...
        $rate_error = check_rate_limit($email, $ip, $conn);
        if ($rate_error) {
            $error = $rate_error;
        } else {
            $sql = "SELECT id, full_name, password, role, is_active FROM users WHERE email = :email LIMIT 1";
            ...
            if ($user && password_verify($password, $user["password"])) {
                ...
                record_failed_login($email, $ip, $conn);
    ...
    <div class="glass-card auth-card">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="email" id="email" name="email" class="form-control" placeholder="example@email.com" autocomplete="email" required>
            </div>

            <div class="form-group">
                <label for="password">Нууц үг</label>
                <input type="password" id="password" name="password" class="form-control" placeholder="Нууц үгээ оруулна уу" autocomplete="current-password" required>
            </div>
